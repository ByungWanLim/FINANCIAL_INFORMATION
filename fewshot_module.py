...
from faiss_module import normalize_string
...
def fewshot_ex(fewshot_db, buff, train_db = None, fewshot_num = 3):
    ...
    ex = example_selector.select_examples({'Question': normalize_string(buff['Question'])}) # buff['Question']에 해당하는 fewshot_num개의 문서를 선택 리턴: [{'Question': '질문', 'Answer': '답변'}]
    fewshot_str = ""
    for i , entry in enumerate(ex):
        train_retriever = train_db.as_retriever(
                search_kwargs={'k':1})
        question = entry['Question']
        retrieved_docs = train_retriever.invoke(normalize_string(question))
