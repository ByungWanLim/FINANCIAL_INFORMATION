...
class FAISSDatabaseManager:
    def __init__(self, db_path, chunk_strategy="paragraph", search_strategy="knn_best_field", embedding_model_name='intfloat/multilingual-e5-small'):
        """
        초기화 메서드.
    ...
    def chunk_text(self, text, max_length=512):
        ...
        if self.chunk_strategy == "paragraph":
            ...
            return self.serm_based_chunking(text, max_length)
        else:
    ...
    def make_db(self, df, max_length=512, embedding_dim=None, fewshot=False):
        ...
        if os.path.exists(self.db_path):
            #logger.info(f"Loading FAISS DB from: {self.db_path}")
            self.load_faiss_db()
            return
